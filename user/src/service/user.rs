...
use crate::common::extract::Multipart;
use anyhow::Result;
use tokio::fs::File;
...
pub async fn upload_avatar(claims: User, mut multipart: Multipart) -> Result<String>{
    if let Some(field) = multipart.inner.next_field().await? {
        ...
        if name.is_none(){
            return Err(ApiErr::Bad(400, "文件名缺失").into());
        }
        let name = name.unwrap();
        let path = Path::new(name);
        let ext = path.extension();
        if ext.is_none(){
            return Err(ApiErr::Bad(400, "无法确认文件类型").into());
        }
        let ext = ext.unwrap();
    ...
    Err(ApiErr::Bad(400, "文件信息缺失").into())
}
...
pub(crate) async fn select_user(user: User) -> Result<crate::model::user::User>{
    ...
    result.ok_or(ApiErr::Bad(404, "无该用户").into())
}
